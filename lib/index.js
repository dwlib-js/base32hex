    ...
    try {
      ...
      for (let j = 0; j < bytes; j++) {
        ...
        if (charIndex === undefined) {
          ...
          throw new RangeError('Invalid Base32Hex encoding');
        }
...
const DecodeInt = encodedInteger => {
  ...
  let leadingZeros = 0;
  while (leadingZeros < length && encodedInteger[leadingZeros] === '0') {
    leadingZeros++;
  }
  let result = 0;
...
const encodeToBytes = (string, options) => {
  const $string = ToString(string);
...
const decode = (encodedString, options) => {
...
const decodeToBytes = (encodedString, options) => {
  const $encodedString = ToString(encodedString);
  let ignorePadding = false;
  let allowConcatenation = false;
  if (options !== undefined) {
...
const encodeBytes = (buffer, options) => {
...
const encodeBytesToString = (buffer, options) => {
  RequireBuffer(buffer);
  let withoutPadding = false;
  if (options !== undefined) {
    RequireOptionsObject(options);
...
const decodeBytes = (buffer, options) => {
  RequireBuffer(buffer);
  let ignorePadding = false;
  let allowConcatenation = false;
  if (options !== undefined) {
...
const decodeBytesToString = (buffer, options) => {
  RequireBuffer(buffer);
  let ignorePadding = false;
  let allowConcatenation = false;
  if (options !== undefined) {
...
const encodeTextToBytes = (text, options) => {
  const $text = ToString(text);
  let withPadding = false;
  if (options !== undefined) {
    RequireOptionsObject(options);
...
const decodeText = (encodedString, options) => {
  const $encodedString = ToString(encodedString);
  let ignorePadding = false;
  let allowConcatenation = false;
  if (options !== undefined) {
...
const decodeBytesToText = (buffer, options) => {
  RequireBuffer(buffer);
  let ignorePadding = false;
  let allowConcatenation = false;
  if (options !== undefined) {
...
if (BigInt) {
  ...
  const DecodeBigInt = encodedInteger => {
    const length = encodedInteger.length;
    if (!length) {
      throw new RangeError('Invalid Base32Hex encoded integer');
    }
    let leadingZeros = 0;
    while (leadingZeros < length && encodedInteger[leadingZeros] === '0') {
      leadingZeros++;
